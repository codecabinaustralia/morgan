                ...
                <div class="w-full bg-white rounded-lg p-4">
                    <div class="flex w-full">
                        <div class="flex-shrink">
                            <h2 class="header-semiBold text-vgrayDark">Manage Team</h2>
                        </div>
                    ...
                    <table class="w-full mt-4">
                        <thead class="w-full bg-gray-100 h-12">
                            <td></td>
                            <th class="text-sm font-medium text-vgray text-left pl-4">User Name</th>
                        ...
                        <tbody>
                            <tr v-if="items.length == 0">
                                <td colspan="6" class="bg-gray-200 h-12 text-center text-sm">No users found</td>
                            </tr>
                            <tr v-if="items.length > 0" class="h-12" v-for="(i, index) in items" :key="index">
                                ...
                                <td class="pl-4 text-sm header-semiBold text-gray-500">
                                    <router-link :to="{ name: 'EditTeam', params:{id: i.id} }" :style="currentTenant.primaryColor ? `color:${currentTenant.primaryColor}`: ''" class="text-vgreen header">
                                ...
                                <td :style="currentTenant.primaryColor ? `color:${currentTenant.primaryColor}`: ''" class="text-sm text-vgreen cursor-pointer hover:underline">
                                    <a href="mailto: josh@jblventures.com.au" class="text-sm truncate">{{ i.email }}</a>
                                </td>
                                ...
                                <td>
                                    <div class="w-full flex">
                                        ...
                                        <div class="flex-shrink mr-1">
                                            <router-link :to="{name: 'EditTeam', params:{id: i.id}}">
...
export default {
    ...
    methods: {
        ...
        async getData() {
            ...
            this.loading = true
            // get data
            const q = query(collection(db, "users"), where("tenantId", "==", this.currentTenant.tenantId));
