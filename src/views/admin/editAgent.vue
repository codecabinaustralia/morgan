<template>
    <div class="w-full flex">
        ...
        <div class="flex-grow bg-gray-100 h-screen">
            <admin-primary />
            <div class="container mx-auto p-10 flex flex-wrap h-screen overflow-y-scroll -mt-20 pt-24 z-10 relative">

                <div v-if="error" class="my-4 w-full bg-red-50 text-red-500 text-sm rounded p-4">
                ...
                <div class="w-3/4 pr-2">
                    <div class="bg-white rounded-lg p-4">
                        <div class="flex-shrink">
                            <h2 class="header-semiBold text-sm text-vgreen">Broker Profile</h2>
                                        ...
                                        <form class="divide-y-blue-gray-200 mt-2 space-y-8 divide-y">
                                            <div class="grid grid-cols-1 gap-y-6 sm:grid-cols-6 sm:gap-x-6">
                                                <div class="sm:col-span-6 mb-6">
                                                    <p class=" text-xs text-vgray">This information will be
                                                ...
                                                <div class="sm:col-span-6">
                                                    <label for="photo"
                                                        class="block text-sm font-medium text-blue-gray-900">Photo</label>
                                                    <div class="mt-1 flex items-center">
                                                        <preloader v-if="imageLoading" />
                                                        <img v-if="!imageLoading && img1" class="inline-block h-12 w-12 rounded-full" :src="img1"
                                                            alt="">
                                                        <div class="ml-4 flex">
                                                            <div
...
export default {
    ...
    methods: {
        ...
        async getItem() {
            this.item = null
            const docRef = doc(db, "agents", this.$route.params.id);
            const docSnap = await getDoc(docRef);

            if (docSnap.exists()) {
                let obj = docSnap.data()
                this.$set(obj, "id", docSnap.id)
                if(obj.avatar) this.img1 = obj.avatar
                this.item = obj
            } else {
        ...
        async saveitem() {
            ...
            await setDoc(doc(db, "agents", this.$route.params.id), payload);
